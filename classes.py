...
class NeuralNet(nn.Module):
    ...
    def forward(self, x: torch.tensor) -> float:
        """
         Method returns probability of class with target equals 1 for input data x
...
class AdaBoost():
    ...
    def criterion_for_weak_classifier(self, weak_classifier: NeuralNet) -> torch.tensor:
        """
            This method implements loss function for weak classifier
    ...
    def train_weak_classifier(self, weak_classifier: NeuralNet, n_steps: int, learning_rate: float) -> None:
        """
            This method implements training of weak classifier
    ...
    def N_(self, weak_classifier: NeuralNet) -> float:
        """
            Weighted proportion of incorrect predictions for weak classifier
    ...
    def compute_alpha(self, weak_classifier: NeuralNet) -> float:
        """
            Method computes coefficent for weak classifier in ensemble
            ...
            `  ----------
                    res: float
                        coefficent for weak classifier in ensemble
            """
        with torch.no_grad():
    ...
    def update_weights(self, weak_classifier) -> None:
        """
            Method update weights in AdaBoost algorithm
    ...
    def main_train(self, layer_length: int, n_steps: int, lr: float) -> None:
        ...
        for i in range(self.T):
            model = self.weak_classifier(10, 15)
            self.train_weak_classifier(model, 50, 1e-2)
